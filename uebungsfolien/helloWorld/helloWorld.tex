...
\lstinputlisting[style=JAVA,title=Diese Beispiel hei\ss t HelloWorld und gibt "Hello World!"{} auf der Konsole aus.]
...
\begin{itemize}
 \item außerhalb des darstellbaren Bereichs: Gibt \code{NaN} oder \code{Inf} zur\"uck
 \item Rechengenauigkeit ca. $10^{-16}$
 \item Vorsicht bei "`Integer-Division"'\\
 \code{double r = 5/2;} $\rightarrow$ \code{r = 2;}
\end{itemize}
\medskip
Verwendung von Konstanten (\code{const})
\begin{itemize}
  \item Der Wert einer mit \code{const} erstellten Variable kann nicht ver\"andert werden.
  \item Eine Deklaration (z.B. \code{const int i;}) ist nicht ausreichend, da sp\"ater kein Wert zugewiesen werden kann.
  \item Beispiel: \code{const int pi=3.14;}
\end{itemize}
\end{frame}
